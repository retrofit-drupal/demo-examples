 ...
 *
 * Deprecated. Use Drupal.url.isLocal() instead.
 *
 * @param url
 *   The URL to be tested.
 *
...
Drupal.overlay.getInternalUrl = function (path) {
  var url = drupalSettings.path.baseUrl + path;
  if (Drupal.url.isLocal(url)) {
    return url;
  }
};
...
Drupal.overlay.eventhandlerOverrideLink = function (event) {
  ...
  if (href != undefined && href != '' && target.protocol.match(/^https?\:/)) {
    ...
    // Open admin links in the overlay.
      ...
      var parentLocation = ($target.hasClass('overlay-restore') && window.location.hash.includes('overlay-context'))
...
Drupal.overlay.eventhandlerOperateByURLFragment = function (event) {
  ...
  var state = window.location.hash.substring('#overlay='.length);
  if (state) {
  ...
  if (internalUrl) {
    ...
    var url = `${internalUrl}?render=overlay`;

    this.open(url);
    this.resetActiveClass(this.getPath(drupalSettings.path.baseUrl + state));
  }
...
Drupal.overlay.eventhandlerSyncURLFragment = function (event) {
  if (this.isOpen) {
    ...
    if (this.getPath(drupalSettings.path.baseUrl + expected) != this.getPath(this.iframeWindow.document.location)) {
...
Drupal.overlay.refreshRegions = function (data) {
  $.each(data, function () {
    ...
    $.each(region_info, function (regionClass) {
      ...
      $.get(drupalSettings.path.baseUrl + drupalSettings.overlay.ajaxCallback + '/' + regionName, function (newElement) {
        $(regionSelector).replaceWith($(newElement));
        Drupal.attachBehaviors($(regionSelector), drupalSettings);
      });
    });
  });
};
...
Drupal.overlay.getPath = function (link, ignorePathFromQueryString) {
  ...
  path = path.replace(new RegExp(drupalSettings.path.baseUrl + '(?:index.php)?'), '');
